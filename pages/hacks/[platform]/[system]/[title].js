...
import { Container, Row, Col, Card, Alert, Table, Image, Button } from 'react-bootstrap'
                            ...
                            <Container>
                                <Row>
                                    <Col xs={12} lg size="lg" className="d-grid my-2">
                                        <Button variant={item.patch_file ? 'primary' : 'muted'} disabled={item.patch_file ? false : true} onClick={() => item.patch_file ? getDownloadLink(item.patch_file.includes('//') ? item.patch_file : `${Setup.CDN_URL}/romhacks/${encodeURIComponent(item.patch_file)}`) : false}>Download</Button>
                                    </Col>
                                    <Col xs={12} lg size="lg" className="d-grid my-2">
...
Hack.getLayout = function getLayout(page) {
